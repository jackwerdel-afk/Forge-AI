(function() {
  ...
  // Try data-site-id attribute, then URL param, then window variable
  var SITE_ID = (scriptTag && scriptTag.getAttribute('data-site-id')) || window.FORGE_AI_SITE_ID;
  
  // Also try reading from script src URL param e.g. ?site=xxx
  if (!SITE_ID && scriptTag && scriptTag.src) {
    var srcUrl = new URL(scriptTag.src);
    SITE_ID = srcUrl.searchParams.get('site') || srcUrl.searchParams.get('id');
  }

  var FORGE_TOKEN = window.FORGE_AI_TOKEN || null;
  if (!SITE_ID && !FORGE_TOKEN) return;

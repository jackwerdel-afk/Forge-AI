  ...
  for (const proxy of proxies) {
    try {
      ...
      if (ct.includes('json')) {
        ...
        html = json.contents || '';
        // Check if blocked
        if (json.status && json.status.http_code === 403) return { html: null, blocked: true };
        if (json.status && json.status.http_code === 429) return { html: null, blocked: true };
      } else {
        html = await res.text();
      }
      ...
      if (
        (lowerHtml.includes('checking your browser') && lowerHtml.includes('cloudflare')) ||
        (lowerHtml.includes('ddos-guard')) ||
        (lowerHtml.includes('please wait') && lowerHtml.includes('captcha')) ||
        (lowerHtml.includes('access denied') && html.length < 1000)
      ) {

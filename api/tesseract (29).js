...
module.exports = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // Fresh service client for data — never has user auth context applied
      // Required to bypass RLS correctly (sb.auth.getUser sets user context on sb)
      const { createClient: createServiceClient } = require('@supabase/supabase-js');
      const sbData = createServiceClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_KEY);

      // Fetch all sites with full data
      const { data: allSites } = await sbData.from('user_sites')
      ...
      const { data: secFindings } = await sbData.from('security_findings')
      ...
      // Fetch team members
      const { data: teamMembers } = await sbData.from('team_members')
        .select('user_id, role, status, created_at')
        .eq('agency_id', dataUserId)
        .eq('status', 'active');

      // Fetch recent alerts
      const { data: recentAlerts } = await sbData.from('site_alerts')
      ...
      const { data: portals } = await sbData.from('client_portals')
        .select('site_url, site_name, client_email, token, created_at, last_accessed_at')
        .eq('user_id', dataUserId)
        .limit(20);

      // Fetch agent jobs
      const { data: agentJobs } = await sbData.from('agent_jobs')
...
- ALWAYS use the site's page_content data (title, H1, text_preview) to write accurate fixes.
- Never write a generic meta description — base it on what the site actually does.
Examples:

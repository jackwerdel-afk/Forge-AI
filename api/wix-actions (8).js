...
async function listPages(apiKey, wixSiteId) {
  ...
  // Fetch published site URLs to get page slugs, then try each as itemId
  // The Wix Item SEO Tags API uses page slugs (URL paths) as itemIds for STATIC_PAGE
  ...
  console.log('Site URLs:', JSON.stringify(urlsData).substring(0, 400));

  const siteUrls = urlsData.urls || [];
  // Find homepage URL to derive the site's base
  const primaryUrl = siteUrls.find(u => u.primary) || siteUrls[0];
  const siteBase = primaryUrl ? primaryUrl.url.replace(/\/$/, '') : null;

  if (!siteBase) throw new Error('Could not determine site URL for page discovery');

  // Fetch sitemap to get all page URLs and slugs
  const sitemapRes = await fetch(siteBase + '/sitemap.xml');
  const sitemapText = await sitemapRes.text();
  console.log('Sitemap preview:', sitemapText.substring(0, 500));

  // Extract URLs from sitemap
  const locMatches = sitemapText.match(/<loc>(.*?)<\/loc>/g) || [];
  const pageUrls = locMatches.map(m => m.replace(/<\/?loc>/g, '').trim());
  console.log('Sitemap page URLs:', pageUrls.slice(0, 5));

  // Convert URLs to slugs (itemIds for Wix STATIC_PAGE)
  const pages = pageUrls.map(url => {
    const path = url.replace(siteBase, '').replace(/^\//, '') || 'home';
    return {
      itemId: path,
      name: sanitize(path || 'home', 200),
      url: sanitize(url, 300)
    };
  });

  console.log('Pages from sitemap:', pages.length, pages.length > 0 ? JSON.stringify(pages[0]) : '');

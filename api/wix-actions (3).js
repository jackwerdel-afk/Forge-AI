...
async function listPages(apiKey, wixSiteId) {
  // Use Published Site URLs endpoint — returns page URLs with IDs for published pages
  const { ok, status, data } = await wixApiCall('GET', WIX_PAGES_BASE, apiKey, wixSiteId, null);

  if (!ok) throw new Error(`Wix Pages API error ${status}: ${data.message || 'Unknown error'}`);

  // Published site URLs response has urls array with pageId and url fields
  const urls = data.urls || data.siteUrls || [];
  console.log('Published URLs response keys:', Object.keys(data));
  console.log('URL count:', urls.length);
  if (urls.length > 0) console.log('First URL sample:', JSON.stringify(urls[0]).substring(0, 200));

  const pages = urls.map(u => ({
    itemId: u.pageId || u.id || null,
    name: sanitize(u.pageName || u.name || u.title || u.url || 'Page', 200),
    url: sanitize(u.url || u.pageUrl || '', 300)
  })).filter(p => p.itemId);

  return { pages, total: pages.length, rawCount: urls.length };
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (tool === 'list_pages') {
      ...
      // If no itemId provided, find the homepage automatically
      if (!itemId) {
        const pagesResult = await listPages(apiKey, wixSiteId);
        const pages = pagesResult.pages || [];
        // Homepage is usually the page with url '/' or named 'Home' or first page
        const home = pages.find(function(p) {
          return p.url === '/' || p.url === '' || (p.name && p.name.toLowerCase() === 'home');
        }) || pages[0];
        if (!home) return res.status(400).json({ error: 'Could not find homepage. Please provide params.itemId.' });
        itemId = home.itemId;
        console.log('Auto-detected homepage itemId:', itemId, 'name:', home.name);
      }

...
module.exports = async (req, res) => {
  // Always respond 200 immediately — Wix retries on non-200
  // Must respond before any async work to prevent Wix timeout
  if (req.method === 'POST') {
  ...
  // Process asynchronously using setImmediate to keep function alive
  setImmediate(async () => {
  try {
    ...
    try {
      ...
      if (parts.length === 3) {
        ...
        // Extract wixUserId from identity field
        let identity = (innerData && innerData.identity) || null;
        if (typeof identity === 'string') {
          try { identity = JSON.parse(identity); } catch(e) {}
        }
        wixUserId = (identity && identity.wixUserId) || null;

        console.log('Wix webhook received — instanceId:', instanceId, 'appId:', appId, 'wixUserId:', wixUserId);
      }
    } catch (parseErr) {
    ...
    // ── ALWAYS FETCH SITE URL FIRST ────────────────────────
    // Do this before the forgeUserId check so we always capture the URL
    let siteUrlAtInstall = null;
    try {
      const installTokenRes = await fetch('https://www.wixapis.com/oauth2/token', {
        method: 'POST',
      ...
      const installTokenData = await installTokenRes.json();
      if (installTokenData.access_token) {
        const urlsRes = await fetch('https://www.wixapis.com/urls-server/v2/published-site-urls', {
          headers: { 'Authorization': `Bearer ${installTokenData.access_token}`, 'Content-Type': 'application/json' }
        });
        const urlsData = await urlsRes.json();
        console.log('Install URLs API status:', urlsRes.status, 'data:', JSON.stringify(urlsData).substring(0, 300));
        const urls = urlsData.urls || [];
        const primary = urls.find(u => u.primary) || urls[0];
        if (primary) siteUrlAtInstall = primary.url.replace(/\/$/, '');
      }
    } catch(urlErr) {
      console.log('Site URL fetch at install (non-fatal):', urlErr.message);
    }
    console.log('Site URL at install:', siteUrlAtInstall);
    ...
    if (!forgeUserId) {
      ...
      try {
        await sb.from('wix_pending_installs').upsert({
          ...
          site_url: siteUrlAtInstall,
          installed_at: new Date().toISOString()
        }, { onConflict: 'instance_id' });
        console.log('Pending install saved — instanceId:', instanceId, 'wixUserId:', wixUserId, 'siteUrl:', siteUrlAtInstall);
      } catch(e) {

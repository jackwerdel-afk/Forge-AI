...
//   3. Match install token from ?token= query param (required)
...
//   - Token match is exact — no time-based fallback (prevents wrong-user bug)
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // ── FIND FORGE USER VIA INSTALL TOKEN ─────────────────
    // Token is always passed as ?token= in the install URL.
    // No fallback — guessing by time causes wrong-user assignment.
    const queryToken = req.query && req.query.token ? req.query.token.trim() : null;

    if (!queryToken) {
      // No token — store as pending for manual resolution
      console.warn('Wix webhook: no token in query — storing as pending install');
      try {
    ...
    console.log('Wix install: matching by token:', queryToken.substring(0, 8) + '...');
    ...
    if (!installToken) {
      console.warn('Wix webhook: token not found or already used — storing as pending');
      try {
        await sb.from('wix_pending_installs').upsert({
          instance_id: instanceId,
          app_id: appId,

...
const { executeTier1Ban, executeTier2Warning, checkUserStatus } = require('./enforce-ban');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!isInternal) {
      ...
      const userStatus = await checkUserStatus(sb, user.id, user.email);
      if (!userStatus.allowed) {
        return res.status(403).json({ error: userStatus.message, reason: userStatus.reason });
      }
      ...
      if (urlCheck) {
        if (urlCheck.tier === 1) {
          await executeTier1Ban(sb, user.id, user.email, cleanUrl, urlCheck.category, urlCheck.reason, ip);
          return res.status(403).json({
            error: 'This site has been flagged for serious policy violations. Your account has been suspended.',
            flagged: true,
            tier: 1
          });
        } else if (urlCheck.tier === 2) {
          const result = await executeTier2Warning(sb, user.id, user.email, cleanUrl, urlCheck.category, urlCheck.reason, ip);
          return res.status(403).json({
            error: result.action === 'suspended'
              ? 'Your account has been suspended for 2 weeks due to repeated policy violations.'
              : 'This site cannot be scanned as it falls under restricted categories.',
            flagged: true,
            tier: 2,
      ...
      if (safeBrowsing && safeBrowsing.flagged) {
        await executeTier1Ban(sb, user.id, user.email, cleanUrl, 'malware', safeBrowsing.reason, ip);
        return res.status(403).json({
          error: 'This site has been flagged by Google Safe Browsing. Your account has been suspended.',
          flagged: true,
          tier: 1
        });
      }
    ...
    // Content moderation check on fetched HTML
    if (!isInternal && currentUser) {
      const contentCheck = checkContent(homeHtml, cleanUrl);
      if (contentCheck) {
        if (contentCheck.tier === 1) {
          await executeTier1Ban(sb, currentUser.id, currentUser.email, cleanUrl, contentCheck.category, contentCheck.reason, ip);
          return res.status(403).json({
            error: 'This site contains content that violates our terms of service. Your account has been permanently suspended.',
            flagged: true,
            tier: 1
          });
        } else if (contentCheck.tier === 2) {
          const result = await executeTier2Warning(sb, currentUser.id, currentUser.email, cleanUrl, contentCheck.category, contentCheck.reason, ip);
          return res.status(403).json({
            error: result.action === 'suspended'
              ? 'Your account has been suspended for 2 weeks due to repeated policy violations.'
              : 'This site cannot be scanned. It falls under a restricted category.',
            flagged: true,
            tier: 2,
            action: result.action
          });

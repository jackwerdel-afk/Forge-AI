...
// ── PAGE FETCHER ───────────────────────────────────────────
// Store response headers for security module
...
async function fetchPage(url) {
  ...
  for (const attempt of attempts) {
    ...
    if (result === 'BLOCKED') return { blocked: true, html: null, failed: false };
    if (result) return { blocked: false, html: result, failed: false };
  }
  return { blocked: false, html: null, failed: true };
}
...
// Protects against memory exhaustion from hostile targets.
// Applied wherever response bodies are consumed.
const MAX_RESPONSE_BYTES = 5 * 1024 * 1024; // 5MB hard limit

// ── FORGE SECURITY SCHEMA VERSION ─────────────────────────
// Increment when checks are added, removed, or scoring changes.
// Allows historical scores to be compared correctly.
const SECURITY_SCHEMA_VERSION = '1.3';
// 1.0 — HTTP headers, SSL, email security, mixed content
// 1.1 — SSL certificate chain validation (expiry, hostname, key size)
// 1.2 — DKIM enumeration, CSP meta tag detection, authoritative DNS
// 1.3 — WordPress checks (XML-RPC, debug.log, user enumeration, version)
...
async function fetchSecurityHeaders(url) {
  ...
  try {
    ...
    clearTimeout(t);
    // Build header map (lowercase keys) — we only need headers, not body
    const headers = {};
    res.headers.forEach((val, key) => { headers[key.toLowerCase()] = val; });
    // Consume and discard body to free connection — we only need headers
...
async function checkSSLAndHTTPS(url) {
  ...
  if (!isHTTPS) {
    findings.push({
      ...
      severity: 'CRITICAL',
      confidence: computeConfidence('http_header'),
  ...
  try {
    ...
    if (!redirectsToHTTPS) {
      findings.push({
        ...
        severity: 'HIGH',
        confidence: computeConfidence('http_endpoint'),
        ...
        try {
          ...
          if (daysUntilExpiry < 0) {
            findings.push({ id: 'ssl_cert_expired', category: 'SSL/TLS',
              ...
              severity: 'CRITICAL', confidence: computeConfidence('tls_handshake'),
            ...
            findings.push({ id: 'ssl_cert_expiring', category: 'SSL/TLS',
              ...
              severity: 'HIGH', confidence: computeConfidence('tls_handshake'),
              evidence: `Expires: ${cert.valid_to}. Days remaining: ${daysUntilExpiry}`,
          ...
          if (authError) {
            ...
            if (errStr.includes('HOSTNAME') || errStr.includes('hostname')) {
              findings.push({ id: 'ssl_hostname_mismatch', category: 'SSL/TLS',
                ...
                severity: 'CRITICAL', confidence: computeConfidence('http_header'),
              ...
              findings.push({ id: 'ssl_self_signed', category: 'SSL/TLS',
                ...
                severity: 'HIGH', confidence: computeConfidence('http_header'),
                evidence: `TLS error: ${authError}`,
...
// ── DNS RESULT CACHE ───────────────────────────────────────
// SPF/DMARC/DKIM records change infrequently — cache for 24 hours.
// Unlike rate limiting, in-memory cache is fine here: a stale DNS
// result causes a minor accuracy lag, not a security failure.
...
function dnsGetCache(key) {
  ...
  if (Date.now() > entry.expiresAt) { dnsCache.delete(key); return null; }
  return entry.value;
}

function dnsSetCache(key, value) {
  dnsCache.set(key, { value, expiresAt: Date.now() + DNS_CACHE_TTL });
}
...
async function dnsResolveText(hostname) {
  ...
  try {
    ...
    if (!data.Answer || data.Answer.length === 0) {
      dnsSetCache(hostname, []);
      return [];
    }
    const results = data.Answer
    ...
    try {
      ...
      const fallbackResults = records.flat();
      dnsSetCache(hostname, fallbackResults);
  ...
  try {
    ...
    if (!spfRecord) {
      findings.push({
        ...
        severity: 'HIGH',
        confidence: computeConfidence('dns_authoritative'),
...
function checkMixedContent(html, url) {
  ...
  if (httpResources.length > 0) {
    findings.push({
      ...
      confidence: computeConfidence('html_regex'),
...
async function checkWordPressSecurity(url, html) {
  ...
  if (versionMatch) {
    ...
    findings.push({
      ...
      severity: 'LOW',
      confidence: computeConfidence('version_string'),
...
// Persists security findings to the database with full history.
// Tracks when issues were first detected, last seen, and resolved.
async function updateSecurityAuditTrail(sb, siteId, userId, currentFindings) {
  ...
  try {
    ...
    // Get existing findings for this site
    const { data: existing } = await sb.from('security_findings')
      .select('id, finding_id, resolved_at, scan_count')
      .eq('site_id', siteId)
      .eq('user_id', userId);

    const existingMap = new Map((existing || []).map(e => [e.finding_id, e]));

    // Upsert current findings — update last_detected_at and scan_count
    for (const finding of currentFindings) {
      const prev = existingMap.get(finding.id);
      await sb.from('security_findings').upsert({
        site_id: siteId,
        user_id: userId,
        ...
        fix: finding.fix || '',
        first_detected_at: prev ? prev.first_detected_at || now : now,
        last_detected_at: now,
        resolved_at: null, // Re-opened if it was resolved
        scan_count: prev ? (prev.scan_count || 0) + 1 : 1
      }, { onConflict: 'site_id,finding_id' });
    }

    // Mark resolved findings — issues that existed before but not in current scan
    for (const [findingId, prev] of existingMap.entries()) {
      if (!currentIds.has(findingId) && !prev.resolved_at) {
        await sb.from('security_findings').update({
          resolved_at: now,
          last_detected_at: now
        }).eq('site_id', siteId).eq('finding_id', findingId);
      }
    }

    console.log(`Security audit trail updated: ${currentFindings.length} active findings for site ${siteId}`);
  } catch(e) {
    console.error('Security audit trail error (non-fatal):', e.message);
    // Non-fatal — don't let audit trail failure break the scan
  }
}
...
async function runSecurityModule(url, html, headers, sb, siteId, userId) {
  ...
  // Update security audit trail asynchronously — non-blocking
  if (sb && siteId && userId) {
    updateSecurityAuditTrail(sb, siteId, userId, allFindings).catch(e => {
...
function getMeta(html, name) {
  ...
  const r1 = new RegExp('<meta[^>]+(?:name|property)=[\x27\x22]' + esc + '[\x27\x22][^>]+content=[\x27\x22]([^\x27\x22]*)[\x27\x22]', 'i');
  const r2 = new RegExp('<meta[^>]+content=[\x27\x22]([^\x27\x22]*)[\x27\x22][^>]+(?:name|property)=[\x27\x22]' + esc + '[\x27\x22]', 'i');
...
function detectCTA(html) {
  const ctaPatterns = [
    /(get started|sign up|start free|try free|free trial|book a demo|request demo|contact us|get a quote|learn more|buy now|shop now|subscribe|join now|download|get access)/gi
  ];
  const matches = [];
...
function countExternalScripts(html, baseUrl) {
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!isInternal) {
      ...
      try {
        const { data: siteRow } = await sb.from('user_sites')
          ...
          .ilike('url', cleanUrl.replace(/\/\//, '%').replace(/\/$/, '%'))
          .maybeSingle();
        if (siteRow) { siteId = siteRow.site_id; userId = siteRow.user_id; }
        // Also check scheduled_sites
        if (!siteId) {
          const { data: schedRow } = await sb.from('scheduled_sites')
            .select('id, user_id').ilike('url', cleanUrl.replace(/\/\//, '%').replace(/\/$/, '%')).maybeSingle();
          if (schedRow) { siteId = schedRow.id; userId = schedRow.user_id; }
        }
      } catch(e) { console.log('siteId fetch error (non-fatal):', e.message); }
    ...
    const [seoRes, mobileRes, uxRes, maintenanceRes, performanceRes, securityRes] = await Promise.allSettled([
      ...
      runSecurityModule(cleanUrl, homeHtml, _lastResponseHeaders, sb, siteId, userId)
    ]);

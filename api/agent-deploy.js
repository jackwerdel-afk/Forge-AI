...
async function applyFix(siteUrl, credentials, fix) {
  ...
  if (issue_type === 'meta_description') {
    const res = await fetch(`${siteUrl}/wp-json/wp/v2/posts/${target_id}`, {
      ...
      body: JSON.stringify({ meta: { _yoast_wpseo_metadesc: proposed_fix, _forge_meta_description: proposed_fix } })
    });
    if (!res.ok) {
      // Try pages endpoint
...
module.exports = async (req, res) => {
  ...
  if (action === 'deploy') {
    ...
    try {
      ...
      const { data: site } = await sb.from('scheduled_sites')
        .select('wp_credentials')
        .eq('user_id', user.id)
        .eq('url', fix.site_url)
        .maybeSingle();

      if (!site || !site.wp_credentials) {
        await sb.from('agent_fixes').update({
          status: 'failed',

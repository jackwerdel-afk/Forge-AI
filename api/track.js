...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Look up userId from user_sites
    let userId = bodyUserId;
    if (!userId) {
      console.log('Looking up siteId:', JSON.stringify(siteId), 'length:', siteId.length);
      ...
      console.log('siteData:', siteData, 'error:', siteErr?.message);
      if (!siteData) return res.status(404).json({ error: 'Site not found', siteId, siteErr: siteErr?.message });

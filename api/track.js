...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Look up userId from user_sites or scheduled_sites
    let userId = bodyUserId;
    if (!userId) {
      // Check user_sites first
      ...
      if (siteData) {
        ...
        const { data: schedData } = await sb
          ...
          .eq('id', siteId)
          .maybeSingle();
        if (!schedData) return res.status(404).json({ error: 'Site not found' });
        userId = schedData.user_id;
      }
    }

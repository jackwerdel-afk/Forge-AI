...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const {
      siteId: rawSiteId, userId: bodyUserId, url, page, referrer,
      device, browser, os, sessionId,
      duration, scrollDepth, isEntry, isExit,
      token: bodyToken
    } = body;

    if ((!rawSiteId && !bodyToken) || !page) {
    ...
    if (!userId) {
      if (siteId) {
        // Look up by site_id in user_sites
        const { data: siteData } = await sb
          .from('user_sites').select('user_id').eq('site_id', siteId).maybeSingle();
        if (siteData) {
          userId = siteData.user_id;
        } else {
          // Fall back to scheduled_sites
          const { data: schedData } = await sb
            .from('scheduled_sites').select('user_id').eq('id', siteId).maybeSingle();
          if (schedData) userId = schedData.user_id;
        }
      }

      // If still no match, try lookup by token + url (WordPress plugin flow)
      if (!userId && bodyToken && url) {
        const { data: tokenData } = await sb
          .from('agency_tokens').select('user_id').eq('token', bodyToken).maybeSingle();
        if (tokenData) {
          userId = tokenData.user_id;
          try {
            const siteOrigin = new URL(url).origin;
            const { data: siteByUrl } = await sb
              .from('user_sites').select('site_id')
              .eq('user_id', userId).ilike('url', siteOrigin + '%').maybeSingle();
            if (siteByUrl) siteId = siteByUrl.site_id;
          } catch(e) {}
        }
      }

      if (!userId) return res.status(404).json({ error: 'Site not found' });
    }

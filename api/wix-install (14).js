...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Primary: match by token passed in the install URL query param
    // Fallback: match by most recent unused token within 30 minutes
    ...
    if (queryToken) {
      // Exact match — guaranteed correct user
      console.log('Wix install: matching by query token:', queryToken.substring(0, 8) + '...');
      tokenQuery = tokenQuery.eq('token', queryToken);
    } else {
      // Fallback: most recent unused token in last 30 mins
      console.log('Wix install: no query token — using time-based lookup');
      const thirtyMinsAgo = new Date(Date.now() - 30 * 60 * 1000).toISOString();
      tokenQuery = tokenQuery.gte('created_at', thirtyMinsAgo)

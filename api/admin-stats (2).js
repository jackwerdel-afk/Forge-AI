...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const usersResult = arguments[arguments.length - 1]; // last allSettled result
    const userEmailMap = {};
    try {
      const { data: { users: authUsers } } = await sb.auth.admin.listUsers({ perPage: 1000 });
      if (authUsers) authUsers.forEach(u => { userEmailMap[u.id] = u.email || u.id.substring(0, 8); });
    } catch(e) { console.log('User email fetch (non-fatal):', e.message); }
    ...
    // All sites for scan health
    const allU = allSites.status === 'fulfilled' ? (allSites.value.data || []) : [];
    const allS = allSchedSites.status === 'fulfilled' ? (allSchedSites.value.data || []).map(s => ({ ...s, score: s.last_score, auto_scan: true })) : [];
    const allSitesCombined = [...allU, ...allS];
    ...
    uSites.forEach(s => {
      urlsSeen.add(s.url);
      if (!agencyMap[s.user_id]) agencyMap[s.user_id] = { userId: s.user_id, email: userEmailMap[s.user_id] || null, sites: [] };
      agencyMap[s.user_id].sites.push({ ...s, score: s.score });
    });

    sSites.forEach(s => {
      if (urlsSeen.has(s.url)) return; // already in user_sites — skip duplicate
      if (!agencyMap[s.user_id]) agencyMap[s.user_id] = { userId: s.user_id, email: userEmailMap[s.user_id] || null, sites: [] };
      agencyMap[s.user_id].sites.push({ ...s, score: s.last_score || null }); // normalize field name
    });
    ...
    return res.status(200).json({
      ...
      scans: {
        total: allSitesCombined.length,
        scannedToday: allSitesCombined.filter(s => s.last_scanned && new Date(s.last_scanned) >= today).length,
        neverScanned: allSitesCombined.filter(s => !s.last_scanned).length,
        avgScore,
        sites: allSitesCombined,
      },

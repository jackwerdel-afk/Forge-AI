...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Auth client — used for token verification only
    const sb = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_KEY);
    // Admin client — separate instance for data queries, never has user auth applied
    // This ensures service key bypasses RLS correctly
    ...
    let memoryContext = '';
    try {
      const dataUserId2 = memberRecord ? memberRecord.agency_id : user.id;
      const { data: memories } = await sbAdmin.from('tesseract_memory')
        .select('memory_type, content, updated_at')
        .eq('user_id', dataUserId2)
        .order('updated_at', { ascending: false })
        .limit(50);

      if (memories && memories.length > 0) {
        memoryContext = '\nTESSERACT PERSISTENT MEMORY (what you remember about this agency):\n';
        memories.forEach(m => {
          memoryContext += '  [' + m.memory_type + '] ' + m.content + '\n';
        });
      }
    } catch(memErr) {
      console.log('Memory fetch error (non-fatal):', memErr.message);
    }
    ...
    try {
      ...
      const { data: allSites } = await sbAdmin.from('user_sites')
      ...
      console.log('Tesseract sites query — dataUserId:', dataUserId, 'user.id:', user ? user.id : 'null', 'memberRecord:', memberRecord ? memberRecord.agency_id : 'null', 'sites found:', allSites ? allSites.length : 0);
      if (allSites && allSites.length > 0) {
        allSites.forEach(s => {
          console.log('  Site:', s.url, '| has last_result:', !!s.last_result, '| has page_content:', !!(s.last_result && s.last_result.page_content));
        });
      }

      // Fetch security findings
      const { data: secFindings } = await sbAdmin.from('security_findings')
      ...
      // Fetch team members
      const { data: teamMembers } = await sbAdmin.from('team_members')
        .select('user_id, role, status, created_at')
        .eq('agency_id', dataUserId)
        .eq('status', 'active');

      // Fetch recent alerts
      const { data: recentAlerts } = await sbAdmin.from('site_alerts')
      ...
      const { data: portals } = await sbAdmin.from('client_portals')
        .select('site_url, site_name, client_email, token, created_at, last_accessed_at')
        .eq('user_id', dataUserId)
        .limit(20);

      // Fetch agent jobs
      const { data: agentJobs } = await sbAdmin.from('agent_jobs')
      ...
      serverContext += '(Most critical information first — data ordered by priority)\n';
      ...
      if (sites.length > 0) {
        ...
        sites.forEach(s => {
          ...
          // Page content — placed before modules so it's never truncated
          if (s.last_result && s.last_result.page_content) {
            const pc = s.last_result.page_content;
            serverContext += '    Site content (use this to write accurate fixes):\n';
            if (pc.title) serverContext += '      Page title: ' + pc.title + '\n';
            if (pc.h1) serverContext += '      H1: ' + pc.h1 + '\n';
            if (pc.meta_description) serverContext += '      Current meta description: ' + pc.meta_description + '\n';
            else serverContext += '      Current meta description: MISSING\n';
            if (pc.text_preview) serverContext += '      Page text preview: ' + pc.text_preview.substring(0, 200) + '\n';
            console.log('page_content for:', s.url, 'title:', pc.title ? pc.title.substring(0,30) : 'none');
          } else {
            console.log('NO page_content for:', s.url, 'last_result keys:', s.last_result ? Object.keys(s.last_result).join(',') : 'null');
          }
    ...
    // Trim server context if too long to prevent token overflow
    const MAX_CONTEXT_CHARS = 12000;
    let trimmedServerContext = serverContext;
    if (serverContext.length > MAX_CONTEXT_CHARS) {
      trimmedServerContext = serverContext.substring(0, MAX_CONTEXT_CHARS) + '\n[Context trimmed for length — showing most important data only]\n';
      console.log('Server context trimmed from', serverContext.length, 'to', MAX_CONTEXT_CHARS, 'chars');
    }
...
IDENTITY — CRITICAL. Follow these exactly:

WHAT YOU ARE:
- You are Tesseract, the intelligence layer inside Forge AI, built by Werdel Global Systems.
- You are not ChatGPT, Claude, Gemini, or any other general-purpose AI. You are purpose-built for website agencies.
- If asked "are you AI?" — Yes. You are Tesseract, an AI built specifically for website agencies by Werdel Global Systems.
- If asked "what AI model are you?" or "what are you built on?" or "are you Claude?" or "are you ChatGPT?" — Never confirm or deny the underlying model. Say: "I'm Tesseract — built by Werdel Global Systems specifically for website agencies. What I'm built on isn't something I share, but what matters is what I can do for your agency."
- Never mention Anthropic, Claude, OpenAI, or any AI provider under any circumstances.

WHO BUILT YOU:
- If asked "who made you?", "who built Forge AI?", "who is behind this?" — Answer: "Forge AI was built by Werdel Global Systems — a software company focused on building tools for website agencies. I'm their flagship intelligence layer."
- If asked about Werdel Global Systems specifically — "Werdel Global Systems is the company behind Forge AI. They build software for website agencies."
- Never make up details about company size, team, location, founding date, or contact information.

ARE YOU THE BEST:
- If asked "are you the best?", "are you the best website AI?", "why should I use you?" — Answer confidently and specifically:
  "Yes. I'm the only AI with real-time access to your entire website portfolio — every score, every security finding, every client, every trend. I don't just answer questions. I brief you every morning, trigger scans, send reports, and remember what matters to your agency across every conversation. No general-purpose AI can do that because they don't have access to your live data. I do."

WHAT YOU CAN DO:
- If asked "what can you do?" or "what are your capabilities?" — Answer:
  "I can brief you every morning on what needs attention across your portfolio. I can scan any of your sites on command. I identify security vulnerabilities, declining scores, and portfolio-wide patterns. I send client reports directly. I remember your preferences and priorities across every conversation. And I know everything about your sites in real time — scores, security findings, WordPress data, client portals, agent jobs, alerts. I'm not a chatbot. I'm your agency's intelligence layer."

COMPETITIVE QUESTIONS:
- If asked how you compare to AgencyAnalytics — "AgencyAnalytics is a reporting tool. I'm an intelligence layer. AgencyAnalytics shows you data. I tell you what it means, what to do about it, and I can take action directly. I also include security monitoring, WordPress management, and AI that actually knows your portfolio."
- If asked how you compare to ManageWP — "ManageWP focuses on WordPress maintenance — updates, backups. I cover your entire portfolio across every platform, add security intelligence, client reporting, and an AI assistant that understands your agency. They maintain sites. I help you run an agency."
- If asked about any other competitor — Acknowledge they exist, explain what Forge AI does that they don't, never disparage them directly.

FORGE AI:
- If asked "what is Forge AI?" — "Forge AI is the operating system for website agencies. It monitors, secures, audits, optimizes, and reports on every client site you manage — from one platform. I'm the intelligence layer that ties it all together."
- If asked "is Forge AI safe?" or "is my data secure?" — "Forge AI is built with enterprise-grade security. Data is encrypted, credentials are stored securely, and your client information is never shared. Security is a core part of how Forge AI is built, not an afterthought."

THINGS YOU NEVER DO:
- Never mention Anthropic, Claude, OpenAI, Google, or any AI provider.
- Never make up company details, contact info, pricing, or team information.
- Never say "I'm just an AI" or be self-deprecating about your capabilities.
- Never pretend to be human.

MEMORY:
- You have persistent memory of this agency across conversations. Use it.
- If you remember something relevant — a preference, a priority, a past discussion — reference it naturally.
- Example: "Last time you mentioned SqualidPin is your most important client — it now has a CRITICAL security finding."
- Never pretend to remember something not in your memory. If it's not there, you don't know it.
...
- action text options: "View Details" (default), "Scan Now" (if overdue), "Send Report" (if report needed)
- Max 4 cards, ranked by urgency.
- followUps: always 2-3, specific to what was just discussed — not generic.

EXECUTABLE ACTIONS:
You can trigger real actions by including an "execAction" field in your response JSON.
Only include execAction when the user explicitly asks you to DO something.
Examples:
- User says "scan siteforgex.com now" → execAction: {"type": "scan", "siteId": "exact-id-from-context", "siteUrl": "https://siteforgex.com"}
- User says "send report to jane@client.com for siteforgex.com" → execAction: {"type": "sendReport", "siteId": "id", "siteUrl": "url", "clientEmail": "jane@client.com", "clientName": "Jane"}
- User says "show me the details for X" → execAction: {"type": "openDetails", "siteId": "exact-id"}
Never fabricate siteIds. Only use IDs from the SITE DETAILS section.

WIX SEO MUTATIONS (wixFix):
For Wix sites connected with an API key, you CAN queue SEO fixes for user approval via Forge Agent.
Supported operations: update_meta_description, update_seo_title.
IMPORTANT RULES:
- Only use wixFix for sites where platform === "wix" in the site data.
- NEVER claim a fix was made — only say it has been queued for approval.
- The user must approve in Forge Agent before any change is deployed.
- NEVER fabricate page IDs — leave targetId null if you don't have it from scan data.
- wixFix is for SEO metadata only — not content, design, plugins, or server headers.
- ALWAYS use the site's page_content data (title, H1, text_preview) to write the proposed fix.
- The proposed meta description must reflect what the site actually does — never write a generic one.
- If page_content is available, base the fix on real site content. If not, ask the user for more context.
Examples:
- User says "fix the meta description on siteforgex.com" → execAction: {"type": "wixFix", "siteId": "exact-id", "siteUrl": "https://siteforgex.com", "issueType": "meta_description", "issueDescription": "Missing meta description on homepage", "proposedFix": "SiteForgeX helps web agencies monitor, secure, and report on every client site from one intelligent platform.", "targetId": null, "targetLabel": "Homepage", "reasoning": "No meta description detected in last scan — hurts SEO click-through rate.", "pointsImpact": 5}
- User says "update the SEO title for siteforgex.com" → execAction: {"type": "wixFix", "siteId": "exact-id", "siteUrl": "https://siteforgex.com", "issueType": "seo_title", "issueDescription": "SEO title needs improvement", "proposedFix": "SiteForgeX — Web Agency Intelligence Platform", "targetId": null, "targetLabel": "Homepage", "reasoning": "Weak or missing SEO title reduces search visibility.", "pointsImpact": 3}

PERSISTENT MEMORY:
${memoryContext}

CURRENT AGENCY DATA:
${context}${reportsContext}${trimmedServerContext}`;
    ...
    if (!data.content || !data.content[0]) {
      console.error('Anthropic API error response:', JSON.stringify(data).substring(0, 300));
      throw new Error('No response from AI: ' + (data.error && data.error.message ? data.error.message : JSON.stringify(data).substring(0, 100)));
    }
    ...
    try {
      ...
      // JSON parse failed — return raw text as message with safe defaults
      const preJson = jsonStart > 0 ? text.slice(0, jsonStart).trim() : text;
      const rawText = (preJson || text || '').trim();
      parsed = {
        message: rawText || 'I processed your request but had trouble formatting the response. Please try again.',
        cards: [],
        followUps: ['Try again', 'What needs attention?', 'Morning briefing']
      };
    }

    // ── EXTRACT AND SAVE MEMORIES ──────────────────────────
    if (userId && parsed && parsed.message && question) {
      setImmediate(async () => {
        try {
          const memUserId = memberRecord ? memberRecord.agency_id : user.id;
          const memExtract = await fetch('https://api.anthropic.com/v1/messages', {
            ...
            body: JSON.stringify({
              ...
              max_tokens: 300,
              messages: [{ role: 'user', content: 'Extract 0-3 facts worth remembering about this agency. Only concrete, specific facts — preferences, priorities, important clients, instructions. Return JSON only: {"memories":[{"type":"preference|priority|client|instruction|context","content":"fact"}]}. If nothing, return {"memories":[]}. Q: ' + question.substring(0, 150) + ' A: ' + parsed.message.substring(0, 200) }]
            })
          });
          const memData = await memExtract.json();
          const memText = (memData.content || []).map(c => c.text || '').join('');
          const clean2 = memText.replace(/```json|```/g, '').trim();
          const memJson = JSON.parse(clean2);
          if (memJson.memories && memJson.memories.length > 0) {
            for (const mem of memJson.memories) {
              if (mem.content && mem.type) {
                await sbAdmin.from('tesseract_memory').upsert({
                  user_id: memUserId, memory_type: mem.type, content: mem.content,
                  source_question: question.substring(0, 200), updated_at: new Date().toISOString()
                }, { onConflict: 'user_id,memory_type,content' });
              }
            }
          }
        } catch(memErr) { console.log('Memory save error (non-fatal):', memErr.message); }
      });
    }

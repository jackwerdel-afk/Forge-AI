...
// Uses Client Credentials flow: App ID + App Secret + instanceId
// Tokens expire in ~4 hours so we get a fresh one each call
...
async function loadCredentials(sb, userId, siteId) {
  ...
  // Support both new (instance_id) and old (encrypted_api_key) credential formats
  if (instance_id) {
    // New format — use Client Credentials flow
    const accessToken = await getAccessToken(instance_id);
    // wix_site_id may not be set yet for new installs — use instance_id as fallback
    const wixSiteId = wix_site_id || instance_id;
    return { accessToken, wixSiteId, credentialType: 'instance' };
  } else if (encrypted_api_key) {
    // Legacy format — decrypt API key
    // Keep for backward compatibility with existing connected sites
...
// ── TOOL: list_pages ──────────────────────────────────────
// siteBase is passed in from user_sites.url — we don't call
// the Wix URLs API because it returns 403 with OAuth tokens
async function listPages(apiKey, wixSiteId, siteBase) {
  ...
  // Build page list from sitemap URLs
  // Normalize both siteBase and page URLs to handle www vs non-www mismatch
  ...
  // ALWAYS probe the homepage first (empty string itemId)
  // This is the most important page and always returns the real Wix itemId
...
async function updateMetaDescription(apiKey, wixSiteId, itemId, newDescription, existingTitle) {
  ...
  console.log('PATCH response status:', status, 'data:', JSON.stringify(data).substring(0, 300));
  if (!ok) throw new Error(`Wix SEO update error ${status}: ${data.message || 'Unknown error'}`);

  // Return patch response data so verify can use it directly
  const patchTags = (data.itemSeoTags && data.itemSeoTags.tags) || [];
  return { updated: true, itemId: cleanItemId, patchData: { tags: patchTags } };
}
...
async function updateSeoTitle(apiKey, wixSiteId, itemId, newTitle, existingDescription) {
  ...
  const patchTags2 = (data.itemSeoTags && data.itemSeoTags.tags) || [];
  return { updated: true, itemId: cleanItemId, patchData: { tags: patchTags2 } };
}
...
async function verifyChange(apiKey, wixSiteId, itemId, field, expectedValue) {
  ...
  // Wait 4s for Wix to propagate the change
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Get site URL from user_sites for listPages (Wix URLs API returns 403)
    ...
    if (tool === 'list_pages') {
      ...
      if (!itemId) {
        ...
        if (!home || !home.itemId) return res.status(400).json({ error: 'Could not find homepage. Please try again.' });
        itemId = home.itemId;
      ...
      // Verify — check PATCH response directly since Wix GET may serve cached data
      // The PATCH response contains the saved tags — verify against that
      const patchResult = result;
      let verification;
      if (patchResult && patchResult.patchData) {
        const savedTags = (patchResult.patchData.tags || []);
        const savedDesc = savedTags.find(t => t.type === 'meta' && t.props && t.props.name === 'description');
        const actualDesc = savedDesc ? savedDesc.props.content : null;
        const verified = actualDesc === newDescription;
        console.log('Verify from PATCH response — expected:', newDescription.substring(0, 50), 'actual:', actualDesc ? actualDesc.substring(0, 50) : 'NULL', 'verified:', verified);
        verification = { verified, field: 'description', expected: newDescription, actual: actualDesc, source: 'patch_response' };
      } else {
        verification = await verifyChange(accessToken, wixSiteId, itemId, 'description', newDescription);
      }
      ...
      if (!itemId) {
        ...
        const home2 = pagesResult2.pages.find(p => p.valid) || pagesResult2.pages.find(p => p.isHomepage) || pagesResult2.pages[0];
        if (!home2 || !home2.itemId) return res.status(400).json({ error: 'Could not find homepage. Please try again.' });
        itemId = home2.itemId;
      ...
      // Verify from PATCH response directly
      let verification;
      if (result && result.patchData) {
        const savedTags = (result.patchData.tags || []);
        const savedTitle = savedTags.find(t => t.type === 'title');
        const actualTitle = savedTitle ? savedTitle.children : null;
        const verified = actualTitle === newTitle;
        verification = { verified, field: 'title', expected: newTitle, actual: actualTitle, source: 'patch_response' };
      } else {
        verification = await verifyChange(accessToken, wixSiteId, itemId, 'title', newTitle);
      }
      result.verification = verification;

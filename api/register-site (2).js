...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Trigger immediate scan so score shows up right away
    // Use fire-and-forget so we don't slow down the response
    const siteIdForScan = siteId;
    fetch('https://forgeai-wgs.com/api/scheduled-scan', {
      ...
      body: JSON.stringify({ singleSite: { url: url, user_id: userId, site_id: siteIdForScan } })
    }).catch(function(e) { console.log('Auto-scan trigger error:', e.message); });

    return res.status(200).json({ success: true, message: 'Site registered and scan triggered' });

...
module.exports = async (req, res) => {
  ...
  try {
    ...
    await sb.from('client_portals').delete().eq('user_id', userId);
    await sb.from('client_reports').delete().eq('user_id', userId);
    await sb.from('site_alerts').delete().eq('user_id', userId);
    await sb.from('uptime_incidents').delete().eq('user_id', userId);
    await sb.from('uptime_checks').delete().eq('user_id', userId);
    await sb.from('security_findings').delete().eq('user_id', userId);
    await sb.from('page_views').delete().eq('user_id', userId);
    await sb.from('site_analytics').delete().eq('user_id', userId);

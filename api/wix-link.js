...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Use site_url saved at install time — most reliable approach
    // The URLs API requires different scopes that aren't available in client_credentials flow
    let siteUrl = pending.site_url || null;
    console.log('Site URL from pending install:', siteUrl);

    if (!siteUrl) {
      return res.status(500).json({ 
        error: 'Could not determine your Wix site URL. Please uninstall and reinstall the Forge AI app, then try again.'
      });
    }

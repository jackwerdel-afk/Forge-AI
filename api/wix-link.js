...
module.exports = async (req, res) => {
  ...
  try {
    ...
    console.log('Token response:', JSON.stringify(tokenData).substring(0, 400));
    ...
    const wixSiteIdFromToken = tokenData.site_id || tokenData.siteId || tokenData.instance?.siteId || null;
    console.log('wixSiteId from token:', wixSiteIdFromToken);

    // ── GET SITE URL FROM WIX ───────────────────────────────
    // Try without wix-site-id first — token is scoped to the instance

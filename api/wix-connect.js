...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'list') {
      ...
      // Wix Studio API does not return domain/URL in site-list response.
      // We return all sites and let the user supply URLs where missing.
      const sites = allSitesRaw.map(function(site) {
        return {
          id: site.id,
          siteId: site.id,
          name: site.displayName || site.name || 'Untitled Site',
          url: (site.url && site.url.startsWith('http')) ? site.url : null,
          published: site.published || false,
          needsUrl: !(site.url && site.url.startsWith('http'))
        };
      });
      ...
      return res.status(200).json({ success: true, sites: finalSites, total: finalSites.length });
    }
    ...
    if (action === 'add') {
      ...
      // siteIds is an array of { id, siteId, url, name } objects passed from frontend
      // The URL was either detected from Wix API or entered manually by the user
      const added = [];
      const failed = [];

      for (const site of siteIds) {
        const url = site.url;
        if (!url) { failed.push(site.name || site.id); continue; }

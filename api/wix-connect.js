...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'list') {
      ...
      console.log('Wix API raw response:', JSON.stringify(wixData).substring(0, 500));
      const allSitesRaw = wixData.sites || [];
      console.log('Total sites from Wix API:', allSitesRaw.length);
      if (allSitesRaw.length > 0) {
        console.log('First site sample:', JSON.stringify(allSitesRaw[0]).substring(0, 300));
      }
      // Fetch domain info for each site from the Wix Domains API
      // This is required for Wix Studio sites which don't include URL in site-list
      const sites = [];
      for (const site of allSitesRaw) {
        let siteUrl = site.url || null;

        // If no URL yet, try the domains API for this specific site
        if (!siteUrl) {
          try {
            // Wix domains endpoint
            const domainRes = await fetch('https://www.wixapis.com/domains/v1/sites/' + site.id + '/domains', {
              method: 'GET',
              headers: {
                'Authorization': apiKey,
                'wix-account-id': accountId,
            ...
            console.log('Domains for', site.name, 'status:', domainRes.status, ':', JSON.stringify(domainData).substring(0, 300));
            if (domainRes.ok) {
              const domains = domainData.domains || domainData.domainNames || [];
              if (domains.length > 0) {
                const primary = domains.find(function(d) { return d.isPrimary || d.primary; }) || domains[0];
                const domainName = primary.domain || primary.domainName || primary.name || (typeof primary === 'string' ? primary : null);
                if (domainName) siteUrl = 'https://' + domainName;
              }
            }
          } catch(domErr) {
            console.log('Domain fetch error for', site.name, ':', domErr.message);
          }
        }
      ...
      const sitesWithUrl = sites.filter(function(s) { return s.url; });
      console.log('Sites with URL after domain lookup:', sitesWithUrl.length);

      if (sitesWithUrl.length === 0) {
        return res.status(200).json({
          sites: [],
          message: 'No sites with a connected domain found. Make sure your Wix sites have a domain connected in the Wix dashboard.',
          debug: { total: allSitesRaw.length, sample: sites[0] || null }
        });
      }
      const finalSites = sitesWithUrl;

      console.log('Wix sites found:', finalSites.length, 'for user:', userId);
      return res.status(200).json({ sites: finalSites, total: finalSites.length });
    }

  ...
  try {
    ...
    if (action === 'list') {
      ...
      for (const site of allSitesRaw) {
        ...
        if (!siteUrl) {
          try {
            const domainRes = await fetch('https://www.wixapis.com/site-properties/v4/properties', {
              method: 'GET',
              headers: {
                'Authorization': apiKey,
                'wix-site-id': site.id,
            ...
            if (domainRes.ok) {
              ...
              console.log('Domain data for', site.name, ':', JSON.stringify(domainData).substring(0, 300));
              // Try various fields
              const props = domainData.properties || domainData;
              siteUrl = (props.url)
                || (props.externalDomain ? 'https://' + props.externalDomain : null)
                || (props.wixDomain ? 'https://' + props.wixDomain : null)
                || (domainData.url)
                || null;
            }
          } catch(domErr) {

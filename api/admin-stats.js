...
module.exports = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      scans: {
        total: uSites.length,
        scannedToday: uSites.filter(s => s.last_scanned && new Date(s.last_scanned) >= today).length,
        neverScanned: uSites.filter(s => !s.last_scanned).length,
        avgScore: uSites.filter(s => s.score !== null).length > 0
          ? Math.round(uSites.filter(s => s.score !== null).reduce((a, s) => a + s.score, 0) / uSites.filter(s => s.score !== null).length)
          : null,
        sites: uSites,
      },

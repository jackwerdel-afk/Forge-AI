  ...
  try {
    ...
    for (const site of paidSites) {
      try {
        ...
        // Set first_scanned_at if this is the first scan
        ...
        // Send monthly report if: has client email, been 30+ days since first scan or last report
        ...
        if (shouldSendReport) {
          try {
            ...
            if (!portalToken) {
              ...
              await sb.from('client_portals').insert({
            ...
            const s = newScore;
            const label = s >= 90 ? 'excellent — performing at a high level across all areas'
                        : s >= 80 ? 'good — in solid shape with a few areas to improve'
                        : s >= 70 ? 'fair — some areas that should be addressed soon'
                        : 'below average — issues that need attention';
            ...
            if (topIssue) {
              const issueText = (topIssue.issue || '')
                .replace(/h1/gi, 'main page heading')
                .replace(/h2/gi, 'section headings')
                .replace(/meta description/gi, 'search result description')
                .replace(/alt text/gi, 'image descriptions')
                .replace(/lcp/gi, 'page load speed');
              summary += 'The top priority for improvement is ' + issueText.toLowerCase() + '.';
            }

            // Get agency name
            const { data: agencyProfile } = await sb.from('agency_tokens')
              .select('agency_name')
              .eq('user_id', site.user_id)
              .maybeSingle();
            const agencyName = agencyProfile?.agency_name || 'Forge AI';
            ...
            await sb.from('scheduled_sites').update({
              last_report_sent_at: nowISO
            }).eq('id', site.id);

            console.log('Monthly report sent to ' + clientEmail + ' for ' + site.url);
          } catch(reportErr) {
            console.log('Monthly report error for ' + site.url + ':', reportErr.message);
          }
        }

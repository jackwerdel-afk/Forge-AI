...
async function listPages(apiKey, wixSiteId) {
  ...
  const url = WIX_SEO_BASE.replace('/item-seo-tags', '') + '/item-seo-tags';
  const listUrl = `${url}?itemType=STATIC_PAGE&paging.limit=50`;
  const { ok, status, data } = await wixApiCall('GET', listUrl, apiKey, wixSiteId, null);

  console.log('List Item SEO Tags status:', status, 'keys:', Object.keys(data || {}));
  if (data && (data.itemSeoTagsList || data.items)) {
    const items = data.itemSeoTagsList || data.items || [];
    console.log('Items count:', items.length);
    if (items.length > 0) console.log('First item:', JSON.stringify(items[0]).substring(0, 300));
  }

  if (!ok) throw new Error(`Wix SEO list error ${status}: ${data.message || 'Unknown error'}`);

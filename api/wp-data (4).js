...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (dataType === 'overview' || !dataType) {
      ...
      // Base plugin list from WordPress REST API
      const basePlugins = plugins.status === 'fulfilled' ? plugins.value.map(p => ({
        name: p.name,
        version: p.version,
        status: p.status,
      ...
      // Fetch plugin update status from Forge AI REST endpoint
      try {
        // Use absolute path for custom namespace
        const baseUrl = siteUrl.replace(/\/$/, '');
        const pluginStatusRaw = await fetch(`${baseUrl}/wp-json/forge-ai/v1/plugin-status`, {
          headers: {
            'Authorization': `Basic ${credentials}`,
            'Content-Type': 'application/json'
          },
        ...
        const pluginStatusRes = pluginStatusRaw.ok ? await pluginStatusRaw.json() : null;
        if (pluginStatusRes && pluginStatusRes.plugins) {
          // Merge update status into base plugin list
          basePlugins.forEach(plugin => {
            const match = pluginStatusRes.plugins.find(p =>
              p.name === plugin.name || p.slug === plugin.name.toLowerCase().replace(/\s+/g, '-')
            );
            if (match) {
              plugin.updateAvailable = match.update_available || false;
              plugin.latestVersion = match.latest_version || null;
            }
          });
          data.outdatedCount = pluginStatusRes.outdated_count || 0;
          data.pluginStatusLastChecked = pluginStatusRes.last_checked || null;
        }
      } catch(e) {
        console.log('Plugin status fetch (non-fatal):', e.message);
      }

      data.plugins = basePlugins;

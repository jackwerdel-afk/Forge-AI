...
module.exports = async (req, res) => {
  ...
  try {
    ...
    console.log('wix-link: token resolved to user:', user.id, user.email);
    ...
    // The token response may include site_id or instance info


    // ── GET SITE URL FROM WIX ───────────────────────────────
    // Use site_url saved at install time if available
    let siteUrl = pending.site_url || null;
    console.log('Site URL from pending install:', siteUrl);

    // If not saved at install time, try to get from request body or site-properties
    if (!siteUrl) {
      // Check if agency provided it in the request body
      siteUrl = (req.body && req.body.siteUrl) || null;
      
      if (!siteUrl) {
        // Try site-properties API as fallback — returns siteDisplayName at least
        try {
          const propsRes = await fetch('https://www.wixapis.com/site-properties/v4/properties', {
            headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' }
          });
          const propsData = await propsRes.json();
          console.log('Site properties:', JSON.stringify(propsData).substring(0, 300));
          // site-properties doesn't return URL — return error asking for it
        } catch(e) {}
        
        return res.status(400).json({ 
          error: 'Please enter your Wix site URL below.',
          needsSiteUrl: true
        });
      }
    }
    ...
    console.log('Site URL resolved:', siteUrl);

    // ── SAVE SITE TO USER_SITES ─────────────────────────────
    let siteId = 'wix_' + Buffer.from(siteUrl).toString('base64').slice(0, 16).replace(/[+/=]/g, '0');
    const siteName = siteUrl.replace(/^https?:\/\//, '');

    // Try upsert by user_id,url first — if site_id conflicts, update existing
    ...
    if (upsertErr) {
      ...
      // If site_id conflict, update the existing row instead
      if (upsertErr.message.includes('duplicate key') || upsertErr.message.includes('unique constraint')) {
        const { error: updateErr } = await sb.from('user_sites')
          .update({ platform: 'wix', auto_scan: true, updated_at: new Date().toISOString() })
          .eq('user_id', forgeUserId)
          .ilike('url', siteUrl + '%');
        if (updateErr) {
          console.error('user_sites update fallback error:', updateErr.message);
          return res.status(500).json({ error: 'Could not save site. Please try again.' });
        }
        // Get the existing site_id for credentials
        const { data: existingSite } = await sb.from('user_sites')
          .select('site_id')
          .eq('user_id', forgeUserId)
          .ilike('url', siteUrl + '%')
          .maybeSingle();
        if (existingSite) siteId = existingSite.site_id;
      } else {
        return res.status(500).json({ error: 'Could not save site. Please try again.' });
      }
    }

...
function userRateLimit(userId) {
  const now = Date.now();
  const window = 60000; // 1 minute
  const max = 20; // 20 requests per minute per user
  const record = userRateLimitStore.get(userId);
  if (!record || now > record.resetAt) {
    userRateLimitStore.set(userId, { count: 1, resetAt: now + window });
    return true;
  }

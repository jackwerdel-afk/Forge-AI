...
module.exports = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      scans: (function() {
        // Merge both tables, deduplicate by URL, normalize score field
        const seenScan = new Set();
        const allSites = [];
        uSites.forEach(s => { seenScan.add(s.url); allSites.push(s); });
        sSites.forEach(s => { if (!seenScan.has(s.url)) allSites.push({ ...s, score: s.last_score || null }); });

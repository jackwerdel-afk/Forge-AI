...
async function detectContext(html, url) {
  try {
    ...
    const prompt = `Analyze this webpage. Return ONLY valid JSON, no markdown.
URL: ${url}
Content: ${textSample}
Return: {"page_type":"homepage|landing|blog|ecommerce|portfolio|service|other","industry":"agency|retail|healthcare|tech|restaurant|finance|education|nonprofit|other","platform":"wordpress|webflow|wix|squarespace|shopify|custom","audit_mode":"full_audit"}`;
...
async function interpretModule(moduleName, findings, context, url) {
  ...
  try {
    const prompt = `You are a ${moduleName} specialist. Analyze these specific findings for a ${context.industry} ${context.page_type} website (${context.platform}) at ${url}.

Findings with evidence:
${findings.map((f, i) => `${i + 1}. ${f.issue}${f.evidence ? ' — Evidence: ' + f.evidence : ''}`).join('\n')}

For each finding provide:
- A specific actionable fix for THIS type of site
- Why this specific issue matters for this industry/type

Return ONLY a JSON array of strings, one per finding, no markdown. Be specific, not generic:
["specific fix for finding 1", "specific fix for finding 2"]`;

...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Only look for tokens created AFTER polling started to avoid detecting old installs
    const body = req.body || {};
    const pollStartedAt = body.pollStartedAt || null;

    let query = sb.from('wix_install_tokens')
      .select('used, forge_user_id, site_url, created_at')
      .eq('forge_user_id', user.id)
      .eq('used', true)
      .order('created_at', { ascending: false })
      .limit(1);

    // Only match tokens created after polling started
    if (pollStartedAt) {
      query = query.gte('created_at', pollStartedAt);
    }

    const { data: installToken } = await query.maybeSingle();

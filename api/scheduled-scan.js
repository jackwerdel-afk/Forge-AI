...
// Called after every scan on a Wix site.
// Checks for fixable SEO issues and creates agent_fixes records
// using page_content for site-aware proposed fixes.
// Only creates fixes if no pending/deployed fix already exists.
async function autoPopulateWixFixes(sb, site, result, score) {
  ...
  // Get page content from scan result for site-aware fixes
  ...
  if (missingMeta && !currentMeta) {
    // Check if fix already exists for this site
    ...
    if (!existingFix) {
      // Write a site-aware proposed meta description using page content
      let proposed = '';
      if (textPreview && textPreview.length > 50) {
        ...
        const cleanText = textPreview
          ...
          .replace(/\s+/g, ' ')
          .trim();
        // Build description from what the site actually says
      ...
      console.log('Wix auto-populate: existing fix found for meta description on', siteUrl, '— skipping');
    }
  }

  // Insert all new fixes
  if (fixes.length > 0) {
...
module.exports = async (req, res) => {
  ...
  // Wix auto-populate mode — triggered by scan.js after a Wix site scan
  const wixAutoPopulate = req.body && req.body.wixAutoPopulate;
  if (wixAutoPopulate) {
    try {
      await autoPopulateWixFixes(sb, wixAutoPopulate.site, wixAutoPopulate.result, wixAutoPopulate.result.overall_score);
      return res.status(200).json({ success: true, message: 'Wix auto-populate complete' });
    } catch(e) {
      console.log('Wix auto-populate handler error:', e.message);
      return res.status(200).json({ success: true, message: 'Wix auto-populate error (non-fatal)' });
    }
  }
  ...
  try {
    ...
    for (let i = 0; i < activeSites.length; i += BATCH_SIZE) {
      ...
      try {
        ...
        // If this is a Wix site with credentials and SEO issues,
        // automatically create agent_fixes records so the agency
        // can approve fixes directly from Forge Agent without
        // going through Tesseract.
        if (site.platform === 'wix') {
          try {
            await autoPopulateWixFixes(sb, site, result, newScore);
          } catch(wixPopErr) {
            console.log('Wix auto-populate error (non-fatal):', wixPopErr.message);
          }
        }

...
module.exports = async (req, res) => {
  ...
  if (singleSite) {
    ...
    try {
      const scanRes = await fetch('https://forgeai-wgs.com/api/scan', {
        ...
        body: JSON.stringify({ url: singleSite.url, userId: singleSite.user_id, internal: true })
      });
      ...
      if (scanData.result) {
        ...
        await sb.from('user_sites').update({
          ...
          scan_count: 1,
          updated_at: nowISO
        }).eq('url', singleSite.url).eq('user_id', singleSite.user_id);
  ...
  try {
    ...
    for (const site of paidSites) {
      try {
        // Respect user's auto_scan setting

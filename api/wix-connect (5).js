  ...
  try {
    ...
    if (action === 'list') {
      ...
      const sites = allSitesRaw.map(function(site) {
        // Try multiple URL fields — Wix Studio may use different fields
        var siteUrl = site.url
          || (site.domains && site.domains[0] && site.domains[0].url)
          || (site.domains && site.domains[0])
          || (site.externalDomain)
          || (site.wixDomain && ('https://' + site.wixDomain))
          || null;
        return {
          id: site.id,
          siteId: site.id,
      ...
      if (sites.length === 0) {
        ...
        return res.status(200).json({
          sites: [],
          message: 'No published sites found on this account.',
          debug: { total: allSitesRaw.length, sample: allSitesRaw[0] || null }
        });
      }

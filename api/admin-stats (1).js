...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Group by user for agencies
    const agencyMap = {};
    uSites.forEach(s => {
      if (!agencyMap[s.user_id]) agencyMap[s.user_id] = { userId: s.user_id, email: userEmailMap[s.user_id] || null, sites: [], schedSites: [] };
      agencyMap[s.user_id].sites.push(s);
    });
    sSites.forEach(s => {
      if (!agencyMap[s.user_id]) agencyMap[s.user_id] = { userId: s.user_id, email: userEmailMap[s.user_id] || null, sites: [], schedSites: [] };
      agencyMap[s.user_id].schedSites.push(s);
    });

...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    try {
      ...
      // Developers only see fixes for sites assigned to them
      if (memberRecord && memberRecord.role === 'developer') {
        const { data: assignedSites } = await sb.from('user_sites')
          .select('url')
          .eq('user_id', ownerUserId);
        // For now show all sites until site assignment is built
        // Future: filter by specifically assigned sites
      }
  ...
  // POST — generate new fix queue (only owner or manager can trigger)
  if (req.method === 'POST') {
    try {
      if (memberRecord && memberRecord.role === 'developer') {
        return res.status(403).json({ error: 'Developers cannot trigger Forge Agent scans.' });
      }

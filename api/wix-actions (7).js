...
async function listPages(apiKey, wixSiteId) {
  ...
  // Try multiple endpoint formats — the List Item SEO Tags API is new (Aug 11 2026)
  // and documentation doesn't show the exact list endpoint path clearly
  const endpointsToTry = [
    `https://www.wixapis.com/seo-metatags-server/v1/item-seo-tags/list?itemType=STATIC_PAGE`,
    `https://www.wixapis.com/seo-metatags-server/v1/item-seo-tags?item_type=STATIC_PAGE`,
    `https://www.wixapis.com/seo-metatags-server/v1/item-seo-tags/STATIC_PAGE`,
  ];

  let ok, status, data;
  for (const listUrl of endpointsToTry) {
    const result = await wixApiCall('GET', listUrl, apiKey, wixSiteId, null);
    ok = result.ok; status = result.status; data = result.data;
    console.log('Tried:', listUrl, '— status:', status, 'keys:', Object.keys(data || {}));
    if (ok) break;
  }

  if (!ok) {
    // Last resort: try POST list
    const postResult = await wixApiCall('POST',
      'https://www.wixapis.com/seo-metatags-server/v1/item-seo-tags/list',
      apiKey, wixSiteId,
      { itemType: 'STATIC_PAGE', paging: { limit: 50 } }
    );
    ok = postResult.ok; status = postResult.status; data = postResult.data;
    console.log('POST list tried — status:', status, 'keys:', Object.keys(data || {}));
    if (!ok) throw new Error(`Wix SEO list error ${status}: ${data.message || 'Could not list pages'}`);
  }

  const items = data.itemSeoTagsList || data.items || data.pages || [];
  console.log('Items found:', items.length, items.length > 0 ? 'First:' + JSON.stringify(items[0]).substring(0, 200) : '');

  const pages = items.map(item => ({
    itemId: item.itemId || item.id || null,
    name: sanitize(item.pageName || item.name || item.itemId || 'Page', 200),
    url: sanitize(item.url || '', 300)
  })).filter(p => p.itemId);

  return { pages, total: pages.length, rawCount: items.length };
}
  ...
  try {
    ...
    if (tool === 'list_pages') {
      ...
      if (!itemId) {
        ...
        const home = pages.find(p => p.url === '/' || p.url === '' || (p.name && p.name.toLowerCase().includes('home'))) || pages[0];
        if (!home) return res.status(400).json({ error: 'Could not find homepage page ID. Please provide params.itemId manually.' });
        itemId = home.itemId;
      ...
      if (!itemId) {
        ...
        const home2 = pages2.find(p => p.url === '/' || p.url === '' || (p.name && p.name.toLowerCase().includes('home'))) || pages2[0];
        if (!home2) return res.status(400).json({ error: 'Could not find homepage page ID. Please provide params.itemId manually.' });
        itemId = home2.itemId;

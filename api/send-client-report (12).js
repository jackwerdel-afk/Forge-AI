...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const { clientEmail, clientName, siteName, siteUrl, score, portalLink, agencyName, summary, grade } = req.body;
    if (!clientEmail || !portalLink) return res.status(400).json({ error: 'Missing required fields' });

    // Fetch lastResult from DB if not passed
    lastResult = req.body.lastResult || null;
    if (!lastResult && req.body.siteId) {
      try {
        const { data: siteRow } = await sb.from('user_sites')
          .select('last_result')
          .eq('site_id', req.body.siteId)
          .maybeSingle();
        if (siteRow && siteRow.last_result) lastResult = siteRow.last_result;
      } catch(e) { console.log('lastResult fetch (non-fatal):', e.message); }
    }
    if (!lastResult && siteUrl) {
      try {
        const { data: siteRow } = await sb.from('user_sites')
          .select('last_result')
          .ilike('url', siteUrl.replace(/https?:\/\//, '%'))
          .maybeSingle();
        if (siteRow && siteRow.last_result) lastResult = siteRow.last_result;
      } catch(e) { console.log('lastResult url fetch (non-fatal):', e.message); }
    }

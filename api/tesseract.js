    ...
    try {
      ...
      if (sites.length > 0) {
        ...
        sites.forEach(s => {
          ...
          // Page content — used for site-aware fix generation
          // Added BEFORE module scores so it's never truncated by context limit
          if (s.last_result && s.last_result.page_content) {
            ...
            console.log('page_content found for site:', s.url, '| title:', pc.title, '| meta:', pc.meta_description ? 'present' : 'missing');
          } else {
            console.log('NO page_content for site:', s.url, '| last_result keys:', s.last_result ? Object.keys(s.last_result).join(',') : 'null');
          }

...
async function listPages(apiKey, wixSiteId) {
  ...
  const urlsRes = await fetch('https://www.wixapis.com/urls-server/v2/published-site-urls', {
    headers: { 'Authorization': `Bearer ${accessToken}`, 'wix-site-id': wixSiteId, 'Content-Type': 'application/json' }
  });
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (tool === 'list_pages') {
      result = await listPages(accessToken, wixSiteId);
    }
      ...
      if (!itemId) {
        const pagesResult = await listPages(accessToken, wixSiteId);
      ...
      // If no itemId provided, call listPages to find the homepage
      if (!itemId) {
        const pagesResult2 = await listPages(accessToken, wixSiteId);

...
const WIX_PAGES_BASE = 'https://www.wixapis.com/site-pages/v2/pages';
...
async function listPages(apiKey, wixSiteId) {
  // Wix Site Pages v2 uses POST with query body
  const url = WIX_PAGES_BASE + '/query';
  const body = { query: { paging: { limit: 50 } } };
  const { ok, status, data } = await wixApiCall('POST', url, apiKey, wixSiteId, body);

  if (!ok) {
    // Fallback: try GET on base URL
    const { ok: ok2, status: s2, data: d2 } = await wixApiCall('GET', WIX_PAGES_BASE, apiKey, wixSiteId, null);
    if (!ok2) throw new Error(`Wix Pages API error ${s2}: ${d2.message || 'Unknown error'}`);
    const pages2 = (d2.pages || d2.items || []).map(p => ({
      itemId: p.id,
      name: sanitize(p.title || p.name || 'Untitled', 200),
      url: sanitize(p.url || p.pageUrl || '', 300)
    }));
    return { pages: pages2, total: pages2.length };
  }

  const pages = (data.pages || data.items || []).map(p => ({
    itemId: p.id,
    name: sanitize(p.title || p.name || 'Untitled', 200),
    url: sanitize(p.url || p.pageUrl || '', 300)
  }));

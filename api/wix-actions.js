...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (tool === 'list_pages') {
      ...
      if (!itemId || !newDescription) return res.status(400).json({ error: 'params.itemId and params.newDescription are required' });
      ...
      if (!itemId || !newTitle) return res.status(400).json({ error: 'params.itemId and params.newTitle are required' });
